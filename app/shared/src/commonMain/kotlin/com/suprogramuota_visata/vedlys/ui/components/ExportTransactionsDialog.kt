...
fun ExportTransactionsDialog(
    ...
    AlertDialog(
        ...
        title = { Text("Eksportuoti transakcijas (išorės tiekėjams)") },
        text = {
            Column(modifier = Modifier.fillMaxWidth()) {
                Text("Pasirinkite JSON failą eksportui.")
                Spacer(modifier = Modifier.height(8.dp))
                
                Button(onClick = {
                    selectedFile = PlatformFilePicker.pickFileToSave(
                        title = "Eksportuoti transakcijas",
                        defaultFileName = "transactions_export.json",
                ...
                Button(
                    onClick = {
                        ...
                        scope.launch {
                            ...
                            try {
                                val jsonString = Json { prettyPrint = true }.encodeToString(kotlinx.serialization.serializer(), transactions)
                                withContext(Dispatchers.IO) {
                                    file.writeText(jsonString)
                                }
                                statusMessage = "Sėkmingai eksportuota ${transactions.size} transakcijų!"
                                isSuccess = true
                            } catch (e: Exception) {
